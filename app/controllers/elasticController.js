...
const EventElasticSearchUtil = require('../utils/elasticsearch/EventElasticSearchUtil')
...
module.exports = {
            ...
            async.waterfall(
                [
                    function(callback) {
                        ...
                        // let newIndexName = custom.MOVIE_INDEX_NAME;
                    ...
                    function(newIndexName, arg2, callback) {
                        ...
                        MovieElasticSearchUtil.toggleAlliasName(newIndexName)
                        .then(result => {
                            callback(null, newIndexName, "dataWithPriceOrder");
                        })
                        .catch(err => {
    ...
    createEventIndex: function(req, res) {
        // console.log('aaaaaaa')
        return new Promise((resolve, reject)=>{
            ...
            async.waterfall(
                [
                    function(callback) {
                        console.log("Bulk Content Elastic Index Step 1: Create New Index with mapping.");
                        let newIndexName = custom.EVENT_INDEX_NAME + "_" + new Date().getTime();
                        // let newIndexName = custom.EVENT_INDEX_NAME;
                        
                        EventElasticSearchUtil.createNewIndex(newIndexName)
                        .then(result => {
                            callback(null, newIndexName);
                        })
                        .catch(err => {
                    ...
                    function(newIndexName, callback) {
                        console.log("Content New Index Name - ", newIndexName);
                        console.log("Bulk Content Elastic Index Step 2: Copy data form DB to Elastic and upload in bulk mode.");
                        EventElasticSearchUtil.copyDataToNewElasticIndexFromDB(newIndexName)
                        .then(result => {
                            callback(null, newIndexName, "dataWithPriceOrder");
                        })
                        .catch(err => {
                    ...
                    function(newIndexName, arg2, callback) {
                        console.log("Bulk Content Elastic Index Step 3: Toggle Content elastic index name with Alias name.");
                        EventElasticSearchUtil.toggleAlliasName(newIndexName)
                        .then(result => {
                            callback(null, newIndexName, "dataWithPriceOrder");
                        })
                        .catch(err => {
    ...
    singleContentCreateUpdate: async(req, res) => {
        if(req.type == 'event'){
            await EventElasticSearchUtil.documentUpdateDelete(req)
            .catch(err => {
                console.log(err); 
            });
